<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:70:"C:\AppServ\www\tmall\public/../application/admin\view\data\manger.html";i:1524212389;s:55:"C:\AppServ\www\tmall\application\admin\view\layout.html";i:1524455925;}*/ ?>
<!doctype html>
<html>
<head>
	<meta charset="UTF-8">
  ...
  <!-- Table -->
   <table class='table table-hover '>
    ...
    <tr id='tr<?php echo $dat['id']; ?>'>
      ...
      <td>
        <a href="javascript:;" class='btn btn-info' onclick='Mdatafu(<?php echo $dat['id']; ?>,this)'>恢复</a> &nbsp;<a href="javascript:;" onclick='Mdel(<?php echo $dat['id']; ?>,this)' class='btn btn-danger'>删除</a> </td>
    </tr>
...
<script>
  function Mdatafu(id,obj){
    $.post('<?php echo url("ajax_Mdatafu"); ?>',{id:id},function(data){
       if(data==1){
  ...
  function Mdel(id,obj){
    ...
    if(a==true){
       $.post('<?php echo url("ajax_Mdel"); ?>',{id:id},function(data){
        if(data==1){
         $(obj).parent().parent().hide();
         tot=parseInt($('#tot').html());
         $('#tot').html(--tot);
  ...
  function MdataAll(){
   ...
   str=arr.join(',',arr);
   $.post('<?php echo url("ajax_MdataAll"); ?>',{str:str},function(data){
      if(data==1){
  ...
  function MdelAll(){
   ...
   str=arr.join(',',arr);
   $.post('<?php echo url("ajax_MdelAll"); ?>',{str:str},function(data){
      if(arr.length==data){
        for(i=0;i<=arr.length;i++){
          $('#tr'+arr[i]).remove();
        }
